...
% Non linear feedback controller is used to make robot follow a circular
% trajectory, it also provides a solution for posture stabilization

clear; close all
addpath('controlUtilities')

opt=odeset('RelTol',1e-2);
[t, q] = ode45(@NL_syst, [0 10*pi], [2 1 pi],opt);
X_R=0.5*sin(t);
Y_R=0.5*cos(t);
...
figure;
plot(t,q);
title('states of system x, y and theta')
figure;
% Car Grphics
b = [-1 -1]';
a = [2 0]';
...
xlabel('x-axis');
ylabel('y-axis');
title('Circular Trajectory using Non-linear feedback controller')
fig = gcf;
