% Mobile Robot platform with nonlinear feedback controller

function [X_dot] = NL_syst(t, q)
  X_R=0.5*sin(t); 
  Y_R=0.5*cos(t); 
  DX_R=0.5*cos(t);
  DY_R=-0.5*sin(t);
  ...
  w = w_d + K2*Vd*(sin(theta_d-theta)/(theta_d-theta))*((Y_R-y)*cos(theta)-(X_R-x)*sin(theta))
%   +K3*(theta_d-theta);
  x_dot     =v*cos(theta);
  y_dot     =v*sin(theta);
  theta_dot =w;
